/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package com.maximilianoborrajo.poo2.tp10.ejercicio2;

/**
 *
 * @author maxim
 */
public class GameInProcessState implements MaquinaState {

    @Override
    public void insertCoin(MaquinaDeVideoJuegos maquina) {
         System.out.println("Juego en progreso. Espera a que termine.");
    }

    @Override
    public void pressStartButton(MaquinaDeVideoJuegos maquina) {
        System.out.println("El juego ya esta en progreso.");
    }
    
}
